    ...
    if (global.chapter == 2 && global.plot >= 15 && global.plot <= 200)
    {
        if (sprite_index == spr_npc_snailcar)
        {
            ...
            else if (talked == 1)
            {
                ...
                var newspawnedcar = krokosha_car_manager.spawn_this_car_if_it_doesnt_exist(x + 33, y + 23, -pi*0.5, "snailcar", 50);
                
                if (newspawnedcar != -1) {
                    if (!krokosha_car_manager.krokosha_player_is_in_car)
        ...
        if (sprite_index == spr_npc_donutcar)
        {
            ...
            else if (talked == 1)
            {
                ...
                var newspawnedcar = krokosha_car_manager.spawn_this_car_if_it_doesnt_exist(x + 33, y + 23, pi*0.5, "donutcar", 70);
        ...
        if (cardialogue == 1)
        {
            if (talked >= 1 && (true)) { // optional todo:  CHECK IF ITS ONE OF THE SELECT CARS
                if (!instance_exists(krokosha_car_manager))
                    instance_create_depth(0, 0, 4000, krokosha_car_manager);
                
                var idthingidk = 1000 + round(abs(x) + abs((abs(x)+1) * y));
                
                var new_x = lerp(bbox_left,bbox_right,0.5);
                var new_y = lerp(bbox_top,bbox_bottom,0.5);
                var newspawnedcar = krokosha_car_manager.spawn_this_car_if_it_doesnt_exist(new_x, new_y, carrot, "eggmancar", idthingidk);
